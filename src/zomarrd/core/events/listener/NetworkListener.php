...
final class NetworkListener implements Listener
{
    ...
    public function onDataPacketSend(DataPacketSendEvent $event): void
    {
        if ($event->getPacket() instanceof ModalFormRequestPacket) {
            ...
            $this->getNetwork()->getTaskScheduler()->scheduleRepeatingTask(new ClosureTask(function () use ($player): void {
                if ($player->isOnline()) {
                    $this->onPacketSend($player, function () use ($player): void {
                        if ($player->isOnline()) {
                            ...
                            if ($this->times_to_request > 1) {
                                ...
                                $handler = null;
                                $handler = $this->getNetwork()->getTaskScheduler()->scheduleRepeatingTask(new ClosureTask(function () use ($player, $times, &$handler): void {
